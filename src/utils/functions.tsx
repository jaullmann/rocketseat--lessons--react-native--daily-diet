...
export function formatAmericanDate(brazilianDate: string): string {
  const day = Number(brazilianDate.split('/')[0].padStart(2, '0'));
  const month = Number(brazilianDate.split('/')[1].padStart(2, '0'));
  const year = Number(brazilianDate.split('/')[2].padStart(4, '0'));
  return `${year}-${month}-${day}`; 
}

export function formatBrazilianDate(americanDate: string): string {
  const year = Number(americanDate.split('-')[0].padStart(2, '0'));
  const month = Number(americanDate.split('-')[1].padStart(2, '0'));
  const day = Number(americanDate.split('-')[2].padStart(4, '0'));
  return `${day}/${month}/${year}`;
}
